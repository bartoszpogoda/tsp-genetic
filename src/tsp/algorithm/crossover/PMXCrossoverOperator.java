...
public class PMXCrossoverOperator implements CrossoverOperator {
	...
	public PathIndividual crossover(PathIndividual firstParent, PathIndividual secondParent) {
		...
		// search for cities in seconds parent's matching section that aren't
		// already in child
		for (int i = matchingSectionBounds.lowerBound; i <= matchingSectionBounds.upperBound; i++) {
			...
			if (!cityAlreadyInChild) {
				int foundPosition = findPositionForValueInSecondParent(matchingSectionBounds, firstParent, secondParent,
						i);
	...
	private int findPositionForValueInSecondParent(MatchingSectionBounds matchingSectionBounds,
			PathIndividual firstParent, PathIndividual secondParent, int currentPositionInSecondParent) {
